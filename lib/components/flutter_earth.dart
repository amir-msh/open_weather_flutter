...
/// I refactored the following repository to make
/// it compatible with dart 3 and null safety:
...
Quaternion quaternionFromTwoVectors(Vector3 a, Vector3 b) {
  ...
  final double angle = math.acos(c);
...
class EulerAngles {
  ...
  EulerAngles toRadians() => EulerAngles(
        radians(yaw),
        radians(pitch),
        radians(roll),
      );

  EulerAngles toDegrees() => EulerAngles(
        degrees(yaw),
        degrees(pitch),
        degrees(roll),
      );
...
class LatLon {
  ...
  LatLon toRadians() => LatLon(radians(latitude), radians(longitude));
  LatLon toDegrees() => LatLon(degrees(latitude), degrees(longitude));
...
class FlutterEarthState extends State<FlutterEarth>
  ...
  Vector3 canvasPointToVector3(Offset point) {
    final x = point.dx - (width ?? 0) / 2;
    final y = point.dy - (height ?? 0) / 2;
    ...
    for (var y = clipRect.top; y < clipRect.bottom; y += 10.0) {
      ...
      for (var x = clipRect.left; x < clipRect.right; x += 10.0) {
        ...
        if ((i == 0 || lastKeys[i - 1] != key) &&
          ...
          list.add(
            Offset(
              point.dx.truncateToDouble(),
              point.dy.truncateToDouble(),
            ),
          );
        }
        lastKeys[i] = key;
  ...
  Mesh initMeshFaces(Mesh mesh, int subdivisionsX, int subdivisionsY) {
    ...
    final List<Polygon?> faces = List<Polygon?>.filled(faceCount, null);
  ...
  void drawTiles(Canvas canvas, Size size) {
    ...
    // FIX: northPoleImage == null exception
    if (widget.showPole && northPoleImage != null) {
      meshList.add(
        buildPoleMesh(math.pi / 2, radians(84), 5, northPoleImage!),
      );
      meshList.add(
        buildPoleMesh(
          -radians(84),
  ...
  void animateCamera({
    ...
    double fallTime = 0;
    if (riseZoom != null) {
      riseTime = Duration.millisecondsPerSecond *
          (riseZoom - zoom!).abs() /
          riseSpeed!;
    }
    riseZoom ??= zoom;
  ...
  void initState() {
    super.initState();
    tiles = List.filled(maxZoom + 1, HashMap<int, Tile>());
